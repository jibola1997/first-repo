...
int y = 0;
	...
	while (1){ 
        while ((n = recv_nonblock(sd,ans,ALEN-1,500)) != recv_nodata) {
            if (n == 0) {
                if(keepalive == 0) {shutdown(sd, SHUT_RDWR);
                close(sd);
                printf("Entire message from server transmitted\n");
                printf("Connection closed by %s.\n", RHOST);
                return 0;} else {
                    printf("Entire message from server transmitted\n");
                   printf("Connection to  %s still open.\n", RHOST);    
                   return 0;break;}
                }
            if (n < 0 ) {
            ...
            if(strlen(message) > 0 && req[strlen(message) - 1] == '\n')
        ...
        fflush(stdout);
             send(sd,message,strlen(message),0);
               send(sd,"\n",1,0);
                printf("message sent\n");y = 1;
          if (async == 1) {printf(">");}      //fgets(req,256,argv[0]);
...
int main (int argc, char** argv, char** envp) {
    ...
    while (hsize == 0 || vsize == 0) {
		...
		// Because they are converted to strings and not long integers
		 ...
		 if (strcmp(com_tok[0], "RPORT") == 0 && strlen(com_tok[1]) > 0)
			{strcpy(RPORT, com_tok[1]);}
  ...
  //  memset(&command[0], 0, sizeof(command));
    ...
    while(1) {
        ...
        if (fgets(command, 128, stdin)) {
            p = strchr(command , '\0');
            l = strchr(command, '!');
            //printf("%li",(p-command+1));
            if((*p == '\0' && (p-command+1)==2))
             {
              *p = 'E';
                        
            } else if( ((p-command+1)==3) && ((l-command+1)==1) )
              {*p = 'E';}
        }
...
//printf(command);
	if (strcmp(real_com[0], "!") == 0){
			//fprintf(stderr, "%s: remote command\n", __FILE__);                  
                        //printf("check one");
                 sol = 1;
                    if (strcmp(real_com[1], "&") == 0) { // background command coming
            #ifdef DEBUG
                    fprintf(stderr, "%s: background command\n", __FILE__);
            #endif
                    bg = 1;
           ...
           // printf("%s", real_com[0]);
                 ...
                 if(  bg == 1) {
		 real_com = real_com + 2; } else { real_com = real_com +1;}
                 //printf("%s", real_com[0]);
...
if(sol) {
        // Process input:
        if (*p =='E') // no command, luser just pressed return
          {  printf("Enter a valid command\n");
            *p = 'J';
         }
